...
namespace Tzkt.Api.Repositories
{
    ...
    {
        public async Task<int> GetDoubleAttestationsCount(
            Int32Parameter? level,
            TimestampParameter? timestamp)
        {
            var sql = new SqlBuilder(@"SELECT COUNT(*) FROM ""DoubleAttestationOps""")
                .Filter("Level", level)
        ...
        public async Task<IEnumerable<DoubleAttestationOperation>> GetDoubleAttestations(string hash, Symbols quote)
        {
            var sql = """
                SELECT      o.*, b."Hash"
                FROM        "DoubleAttestationOps" as o
            ...
            return rows.Select(row => new DoubleAttestationOperation
            {
                Id = row.Id,
                Level = row.Level,
                Block = row.Hash,
        ...
        public async Task<IEnumerable<DoubleAttestationOperation>> GetDoubleAttestations(Block block, Symbols quote)
        {
            var sql = """
                SELECT      *
                FROM        "DoubleAttestationOps"
                WHERE       "Level" = @level
                ORDER BY    "Id"
                """;
        ...
        public async Task<IEnumerable<Activity>> GetDoubleAttestationOpsActivity(
            ...
            Symbols quote)
        {
            ...
            {
                if (account is not RawDelegate baker || baker.DoubleAttestationCount == 0)
                    continue;
            ...
            return await GetDoubleAttestations(
                or,
        ...
        public async Task<IEnumerable<DoubleAttestationOperation>> GetDoubleAttestations(
            OrParameter? or,
        ...
        public async Task<object?[][]> GetDoubleAttestations(
            AnyOfParameter? anyof,
            AccountParameter? accuser,
            AccountParameter? offender,
            Int64Parameter? id,
            ...
            Symbols quote)
        {
            ...
            var sql = new SqlBuilder($@"SELECT {string.Join(',', columns)} FROM ""DoubleAttestationOps"" as o {string.Join(' ', joins)}")
                .Filter(anyof, x => x == "accuser" ? "AccuserId" : "OffenderId")
                .Filter("AccuserId", accuser, x => "OffenderId")
                .Filter("OffenderId", offender, x => "AccuserId")
                .FilterA(@"o.""Id""", id)
