...
namespace Tzkt.Api.Repositories
{
    ...
    {
        public async Task<int> GetAttestationRewardsCount(
            Int32Parameter? level,
            TimestampParameter? timestamp)
        {
            var sql = new SqlBuilder(@"SELECT COUNT(*) FROM ""AttestationRewardOps""")
                .Filter("Level", level)
        ...
        public async Task<AttestationRewardOperation?> GetAttestationReward(long id, Symbols quote)
        {
            var sql = $@"
                SELECT      o.*, b.""Hash""
                FROM        ""AttestationRewardOps"" as o
            ...
            return new AttestationRewardOperation
            {
                Id = row.Id,
                Level = row.Level,
                Block = row.Hash,
        ...
        public async Task<IEnumerable<Activity>> GetAttestationRewardOpsActivity(
            ...
            Symbols quote)
        {
            ...
            {
                if (account is not RawDelegate baker || baker.AttestationRewardsCount == 0)
                    continue;
            ...
            return await GetAttestationRewards(
                or,
                null, null, null,
                timestamp,
                pagination.sort,
        ...
        public async Task<IEnumerable<AttestationRewardOperation>> GetAttestationRewards(
            OrParameter? or,
            Int64Parameter? id,
            ...
            Symbols quote)
        {
            var sql = new SqlBuilder(@"SELECT o.*, b.""Hash"" FROM ""AttestationRewardOps"" AS o INNER JOIN ""Blocks"" as b ON b.""Level"" = o.""Level""")
                .FilterA(or)
                .FilterA(@"o.""Id""", id)
            ...
            return rows.Select(row => new AttestationRewardOperation
            {
                Id = row.Id,
                Level = row.Level,
                Block = row.Hash,
        ...
        public async Task<object?[][]> GetAttestationRewards(
            Int64Parameter? id,
            AccountParameter? baker,
            Int32Parameter? level,
            TimestampParameter? timestamp,
            ...
            Symbols quote)
        {
            ...
            var sql = new SqlBuilder($@"SELECT {string.Join(',', columns)} FROM ""AttestationRewardOps"" as o {string.Join(' ', joins)}")
                .FilterA(@"o.""Id""", id)
                .FilterA(@"o.""BakerId""", baker)
                .FilterA(@"o.""Level""", level)
                .FilterA(@"o.""Level""", timestamp)
