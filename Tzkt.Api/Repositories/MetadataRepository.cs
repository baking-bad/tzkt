...
namespace Tzkt.Api.Repositories
{
    public class MetadataRepository : DbConnection
    {
        ...
        public async Task Update(string table, string key, List<Metadata<string>> metadatas)
        {
            ...
            foreach (var metadata in metadatas)
            {
                ...
                var rows = await db.ExecuteAsync(upd, new {metadata = metadata.Met, key = metadata.Key});
            }
