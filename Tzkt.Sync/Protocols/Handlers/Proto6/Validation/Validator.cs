    ...
    class Validator : IValidator
    {
        ...
        public async Task<IBlock> ValidateBlock(IBlock block)
        {
            ...
            if (rawBlock.Metadata.BalanceUpdates.Count > 0)
            {
                ...
                if (Cycle >= (Protocol.PreservedCycles + 2) && rawBlock.Operations[0].Count > 0)
                {
                    ...
                    if (rewardsUpdate.Delegate != rawBlock.Metadata.Baker ||
                        rewardsUpdate.Change != GetBlockReward(rawBlock.Operations[0].Sum(x => (x.Contents[0] as RawEndorsementContent).Metadata.Slots.Count), rawBlock.Header.Priority))
            ...
            if (endorsement.Metadata.BalanceUpdates.Count > 0)
            {
                ...
                if (Cycle >= (Protocol.PreservedCycles + 2))
                {
                    ...
                    if (rewardsUpdate.Delegate != endorsement.Metadata.Delegate ||
                        rewardsUpdate.Change != GetEndorsementReward(endorsement.Metadata.Slots.Count, rawBlock.Level < 1 ? lastBlock.Priority : rawBlock.Header.Priority))
        ...
        protected async Task ValidateTransaction(RawTransactionContent transaction, RawBlock rawBlock)
        {
            ...
            if (transaction.Metadata.InternalResults?.Count > 0)
            {
                ...
                foreach (var internalContent in transaction.Metadata.InternalResults.Where(x => x is RawInternalDelegationResult))
                {
                    ...
                    if (!await Cache.AccountExistsAsync(internalDelegation.Source, AccountType.Contract))
                        throw new ValidationException("unknown source contract");
        ...
        void ValidateFeeBalanceUpdates(List<IBalanceUpdate> updates, string baker, string sender, long fee, int cycle)
        {
            ...
            if (updates.Count > 0)
            {
                ...
                if (!updates.Any(x => x is FeesUpdate update && update.Change == fee && update.Delegate == baker && update.Cycle == cycle))
                    throw new ValidationException("invalid fee balance updates");
            }
        }

        void ValidateTransferBalanceUpdates(List<IBalanceUpdate> updates, string sender, string receiver, long amount, long storageFee, long allocationFee, string parent = null)
        {
            if (updates.Count != (amount != 0 ? 2 : 0) + (storageFee != 0 ? 1 : 0) + (allocationFee != 0 ? 1 : 0))
            ...
            if (allocationFee > 0)
            {
                if (!updates.Any(x => x is ContractUpdate update && update.Change == -allocationFee && update.Contract == (parent ?? sender)))
                    throw new ValidationException("invalid transfer balance updates");
            }
        }
        ...
        long GetBlockReward(int endorsements, int priority)
            => (priority == 0 ? Protocol.BlockReward0 : Protocol.BlockReward1) * endorsements;
    }
}
