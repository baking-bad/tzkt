...
namespace Mvkt.Api.Repositories
{
    public partial class OperationRepository
    {
        public async Task<bool?> GetTxRollupCommitStatus(string hash)
        {
            ...
            return await GetStatus(db, nameof(MvktContext.TxRollupCommitOps), hash);
        }
        ...
        {
            var sql = new SqlBuilder(@"SELECT COUNT(*) FROM ""TxRollupCommitOps""")
                .Filter("Level", level)
        ...
        public async Task<IEnumerable<TxRollupCommitOperation>> GetTxRollupCommitOps(string hash, Symbols quote)
        {
            var sql = @"
                SELECT      o.*, b.""Hash""
                FROM        ""TxRollupCommitOps"" as o
            ...
            return rows.Select(row => new TxRollupCommitOperation
            {
                Id = row.Id,
                Level = row.Level,
                Block = row.Hash,
        ...
        public async Task<IEnumerable<TxRollupCommitOperation>> GetTxRollupCommitOps(string hash, int counter, Symbols quote)
        {
            var sql = @"
                SELECT      o.*, b.""Hash""
                FROM        ""TxRollupCommitOps"" as o
        ...
        public async Task<IEnumerable<TxRollupCommitOperation>> GetTxRollupCommitOps(Block block, Symbols quote)
        {
            var sql = @"
                SELECT      o.*
                FROM        ""TxRollupCommitOps"" as o
        ...
        public async Task<IEnumerable<TxRollupCommitOperation>> GetTxRollupCommitOps(
        ...
        public async Task<object[][]> GetTxRollupCommitOps(
            AccountParameter sender,
            AccountParameter rollup,
            Int32Parameter level,
            DateTimeParameter timestamp,
            ...
            Symbols quote)
        {
            ...
            var sql = new SqlBuilder($@"SELECT {string.Join(',', columns)} FROM ""TxRollupCommitOps"" as o {string.Join(' ', joins)}")
                .Filter("SenderId", sender)
                .Filter("RollupId", rollup)
                .FilterA(@"o.""Level""", level)
                .FilterA(@"o.""Timestamp""", timestamp)
