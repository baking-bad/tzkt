...
namespace Mvkt.Api.Repositories
{
    public partial class OperationRepository
    {
        public async Task<bool?> GetTxRollupSubmitBatchStatus(string hash)
        {
            ...
            return await GetStatus(db, nameof(MvktContext.TxRollupSubmitBatchOps), hash);
        }
        ...
        public async Task<IEnumerable<TxRollupSubmitBatchOperation>> GetTxRollupSubmitBatchOps(string hash, Symbols quote)
        {
            var sql = @"
                SELECT      o.*, b.""Hash""
                FROM        ""TxRollupSubmitBatchOps"" as o
            ...
            return rows.Select(row => new TxRollupSubmitBatchOperation
            {
                Id = row.Id,
                Level = row.Level,
                Block = row.Hash,
        ...
        public async Task<IEnumerable<TxRollupSubmitBatchOperation>> GetTxRollupSubmitBatchOps(string hash, int counter, Symbols quote)
        {
            var sql = @"
                SELECT      o.*, b.""Hash""
                FROM        ""TxRollupSubmitBatchOps"" as o
        ...
        public async Task<IEnumerable<TxRollupSubmitBatchOperation>> GetTxRollupSubmitBatchOps(Block block, Symbols quote)
        {
            var sql = @"
                SELECT      o.*
                FROM        ""TxRollupSubmitBatchOps"" as o
        ...
        public async Task<IEnumerable<TxRollupSubmitBatchOperation>> GetTxRollupSubmitBatchOps(
        ...
        public async Task<object[][]> GetTxRollupSubmitBatchOps(
            AccountParameter sender,
            AccountParameter rollup,
            Int32Parameter level,
            DateTimeParameter timestamp,
            ...
            Symbols quote)
        {
            ...
            var sql = new SqlBuilder($@"SELECT {string.Join(',', columns)} FROM ""TxRollupSubmitBatchOps"" as o {string.Join(' ', joins)}")
                .Filter("SenderId", sender)
                .Filter("RollupId", rollup)
                .FilterA(@"o.""Level""", level)
                .FilterA(@"o.""Timestamp""", timestamp)
