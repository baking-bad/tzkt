...
namespace Mvkt.Api.Repositories
{
    public partial class OperationRepository
    {
        public async Task<bool?> GetTxRollupDispatchTicketsStatus(string hash)
        {
            ...
            return await GetStatus(db, nameof(MvktContext.TxRollupDispatchTicketsOps), hash);
        }
        ...
        {
            var sql = new SqlBuilder(@"SELECT COUNT(*) FROM ""TxRollupDispatchTicketsOps""")
                .Filter("Level", level)
        ...
        public async Task<IEnumerable<TxRollupDispatchTicketsOperation>> GetTxRollupDispatchTicketsOps(string hash, Symbols quote)
        {
            var sql = @"
                SELECT      o.*, b.""Hash""
                FROM        ""TxRollupDispatchTicketsOps"" as o
            ...
            return rows.Select(row => new TxRollupDispatchTicketsOperation
            {
                Id = row.Id,
                Level = row.Level,
                Block = row.Hash,
        ...
        public async Task<IEnumerable<TxRollupDispatchTicketsOperation>> GetTxRollupDispatchTicketsOps(string hash, int counter, Symbols quote)
        {
            var sql = @"
                SELECT      o.*, b.""Hash""
                FROM        ""TxRollupDispatchTicketsOps"" as o
        ...
        public async Task<IEnumerable<TxRollupDispatchTicketsOperation>> GetTxRollupDispatchTicketsOps(Block block, Symbols quote)
        {
            var sql = @"
                SELECT      o.*
                FROM        ""TxRollupDispatchTicketsOps"" as o
        ...
        public async Task<IEnumerable<TxRollupDispatchTicketsOperation>> GetTxRollupDispatchTicketsOps(
        ...
        public async Task<object[][]> GetTxRollupDispatchTicketsOps(
            AccountParameter sender,
            AccountParameter rollup,
            Int32Parameter level,
            DateTimeParameter timestamp,
            ...
            Symbols quote)
        {
            ...
            var sql = new SqlBuilder($@"SELECT {string.Join(',', columns)} FROM ""TxRollupDispatchTicketsOps"" as o {string.Join(' ', joins)}")
                .Filter("SenderId", sender)
                .Filter("RollupId", rollup)
                .FilterA(@"o.""Level""", level)
                .FilterA(@"o.""Timestamp""", timestamp)
